...
class GetFreeProxy(object):
    ...
    def freeProxySecond(count=20):
        ...
        urls = [
            ...
            "http://www.66ip.cn/nmtq.php?getnum=&isp=0&anonymoustype=0&start=&ports=&export=&ipaddress=&area=1&proxytype=2&api=66ip"
        ]
        request = WebRequest()
        for _ in urls:
    ...
    def xdaili():
        html = get_page(
            'http://api.xdaili.cn/xdaili-api//greatRecharge/getGreatIp?spiderId=49da9f5556e04f929e4469f3baeb2f19&orderno=YZ20194207065cuSreU&returnType=2&count=13')
        if html:
    ...
    def mimvp():
        html = get_page(
            'https://proxyapi.mimvp.com/api/fetchopen.php?orderid=863216215081231392&num=100&country=%E4%B8%AD%E5%9B%BD&http_type=2&ping_time=1&transfer_time=5&request_method=3&result_sort_field=2&result_format=json')
        if html:
            result = json.loads(html)
            proxies = result.get('result')
            for proxy in proxies:
                yield proxy.get('ip:port')
...
if __name__ == '__main__':
    ...
    CheckProxy.checkGetProxyFunc(GetFreeProxy.mimvp)
